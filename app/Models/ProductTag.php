        ...
        $data = $dataTable->eloquent($taxonomy)
            ...
            ->addColumn( 'action', function ($instance) {
                ...
                if (Gate::allows('product-tag.edit')) {
                    $html .= '<a href="'.route('product-tag.edit', $instance->id ).'" class="btn btn-xs btn-primary btnEdit" style="margin-right: 5px"><i class="fa fa-fw fa-edit"></i> Sửa</a>';
                }

                if (Gate::allows('product-tag.delete')) {
                    $html .= '<a data-href="'.route('product-tag.destroy', $instance->id ).'" data-toggle="modal" data-target="#modal-delete" class="btn btn-xs btn-danger btnDelete"><i class="fa fa-fw fa-trash-o"></i> Xóa</a>';
                }
